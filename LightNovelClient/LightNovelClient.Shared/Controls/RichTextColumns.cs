...
namespace LightNovel.Controls
{
    ...
    public sealed class RichTextColumns : Panel, IScrollSnapPointsInfo
    {
        ...
        public ScrollViewer ViewPortScrollViewer
        {
            get
            {
                if (_scrollViewer == null)
                    _scrollViewer = VisualTreeExtensions.GetParentByType<ScrollViewer>(this);
                return _scrollViewer;
            }
        }

        public double ViewportHorizentalOffset => ViewPortScrollViewer.HorizontalOffset;

        public double ViewportVerticalOffset => ViewPortScrollViewer.VerticalOffset;

        public double ViewportWidth => ViewPortScrollViewer.ViewportWidth;

        public double ViewportHeight => ViewPortScrollViewer.ViewportHeight;

        public int FirstVisiblePageNo
        {
            get;
        }

        public int FirstVisibleLineNo
        {
            get;
        }
        /// <summary>
        ...
        private List<FrameworkElement> _Pages = null;

        private Size _measuredSize;
        private double _measuredContainerSize;
        private double _measuredColumnSize;
        /// <summary>
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            //double offset = 0;
            //int stIdx = 0;
            //int length = ColumnsPerPage * 3;
            //if (Orientation == Windows.UI.Xaml.Controls.Orientation.Horizontal)
            //{
            //    offset = ViewportHorizentalOffset;
            //    stIdx = Math.Max(0, (int)(offset / ColumnWidth) - ColumnsPerPage);
            //}
            //else
            //{
            //    offset = ViewportVerticalOffset;
            //}

            //if (lastIdx > 0)
            //    for (int i = lastIdx; i < stIdx; i++)
            //    {
            //        this.Children.RemoveAt(i - lastIdx);
            //    }

            //if (lastIdx < 0)
            //    lastIdx = -length;
            //for (int i = lastIdx + length; i < stIdx + length; i++)
            //{
            //    Children.Add(_Pages[i]);
            //    _Pages[i].Arrange(new Rect(i * ColumnWidth, 0, ColumnWidth, finalSize.Height));
            //}
