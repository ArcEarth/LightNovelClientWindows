...
namespace LightNovel.Controls
{
	...
	public class ExpanderView : HeaderedItemsControl
	{
		...
		/// <summary>
		/// Indicate weather use animation transitions in Visual state changes, automaticlly set to true in visiable time , set to false in construction and destruction
		/// </summary>
		...
		internal void AnimateContainerDropDown()
		{
			...
			{
				FrameworkElement container = this.ContainerFromIndex(i) as FrameworkElement;
				...
				Storyboard.SetTargetProperty(transAnimation, "(TranslateTransform.Y)");
				...
				Storyboard.SetTargetProperty(opacityAnimation, "(FrameworkElement.Opacity)");
