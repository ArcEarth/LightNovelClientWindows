    ...
    public class ReadingPageViewModel : INotifyPropertyChanged
    {
        ...
        async void ReadingPageViewModel_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            switch (e.PropertyName)
            {
                case "SeriesId":
                    ...
                    {
                        _client = await CachedClient.GetForSeriesAsync(SeriesId.ToString());
                        SeriesData = _client.Index;
                    }
                    break;
                case "VolumeNo":
                ...
                case "ChapterNo":
                    ...
                    {
                        ChapterData = await _client.GetChapterAsync(VolumeData.Chapters[ChapterNo].Id, VolumeData.Id, SeriesData.Id);
        ...
        public Series SeriesData
        {
            ...
            private set
            {
                ...
                {
                    _SeriesData = value;
                    int id = int.Parse(_SeriesData.Id);
                    if (_SeriesId != id)
                        _SeriesId = id;
                    ...
                    Index = (from vol in _SeriesData.Volumes
                             select new VolumeViewModel
                             {
                                 ...
                                 Chapters = vol.Chapters.Select(chapter => new ChapterPreviewModel
                                 {
                                     ...
                                     Id = chapter.Id,
                                     IsDownloaded = _client.IsChapterCached(chapter.Id),
                                     Title = chapter.Title,
        ...
        public RelayCommand LoadNextChapterCommand
        {
            get
            {
        ...
        public int ChapterNo
        {
            ...
            private set
            {
                if (_ChapterNo != value)
                {
                    ...
                    cvm.IsDownloaded = _client.IsChapterCached(cvm.Id);
                    NotifyPropertyChanged();
        ...
        public bool IsDownloaded
        {
            get
            {
                ...
                {
                    if (!_client.IsChapterCached(VolumeData.Chapters[i].Id))
                        return false;
                }
                for (int i = VolumeNo + 1; i < SeriesData.Volumes.Count; i++)
                {
                    if (!SeriesData.Volumes[i].Chapters.All(c => _client.IsChapterCached(c.Id)))
                        return false;
                }
                return true;
            }
            ...
            try
            {
                if (nav.ChapterId != null && nav.VolumeId == null && nav.SeriesId == null)
                {
                    if (nav.SeriesId == null)
                    {
                        ...
                        var chapter = await _client.GetChapterAsync(nav.ChapterId, "-1", "-1");
                    ...
                    _client = await CachedClient.GetForSeriesAsync(nav.SeriesId);
                    var series = _client.Index;
                    if (c.IsCancellationRequested)
                    {
                        IsLoading = false;
                        return;
                ...
                else if (nav.VolumeId != null)
                {
                    ...
                    {
                        // Refresh the index
                        series = await _client.GetIndexAsync(true);
                        if (c.IsCancellationRequested)
                        {
                            IsLoading = false;
                            return;
        ...
        public async Task LoadDataInternalAsync(CancellationToken c, int seriesId, int volumeNo = 0, int chapterNo = 0, int lineNo = 0, PreCachePolicy preCachePolicy = PreCachePolicy.CacheNext)
        {
            ...
            try
            {
                ...
                if (chapterNo >= 0 && (ChapterData == null || ChapterData.Id != VolumeData.Chapters[chapterNo].Id))
                {
                    ...
                    {
                        chapter = await _client.GetChapterAsync(VolumeData.Chapters[chapterNo].Position, true);
                        if (c.IsCancellationRequested)
                        {
                            IsLoading = false;
                            return;
        ...
        public void SyncChapterPreviewDownloadProgress()
        {
            if (Index == null)
                return;
            foreach (var vm in Index)
            {
                foreach (var cpvm in vm.Chapters)
                {
                    cpvm.IsDownloaded = _client.IsChapterCached(cpvm.Id);
                }
            }
        }
        ...
        public async Task<bool> RefreshSeriesDataAsync()
        {
            ...
            try
            {
                SeriesData = await _client.GetIndexAsync(true);
                VolumeData = SeriesData.Volumes[VolumeNo];
                IsLoading = false;
                return true;
            }
        ...
        public async Task<bool?> CachingRestChaptersAsync(bool cacheImages = true)
        {
            ...
            DownloadingTask = _client.CacheChaptersAsync(chapters, cacheImages);
