...
namespace LightNovel.Data
{
    public class CachedClient
    {
        ...
        private async Task<IRandomAccessStream> LoadIllustrationAsyncInternal(CancellationToken c, IProgress<int> progress, string img_url, ulong expectedSize, bool forceRefresh)
        {
            ...
            IRandomAccessStream imageStream = null; // return value
            progress.Report(0);
            ...
            if (file != null)
            {
                ...
                // Image is corrupted
                if ((expectedSize > 0 && prop.Size < expectedSize) || prop.Size == 0)
                    file = null;
            }

            // Try to download the image file
            if ((file == null || forceRefresh)
                && AppGlobal.NetworkState != AppGlobal.AppNetworkState.Unconnected)
            {
                try
                {
                    using (var client = new HttpClient())
                    {
                        ...
                        download.Progress = (info, p) =>
                        {
                            ...
                            if (p.TotalBytesToReceive.HasValue)
                                expectedSize = p.TotalBytesToReceive.Value;
                            if (expectedSize > 0)
                        ...
                        bool corrupted = !succ || recieved < expectedSize && expectedSize != 0;
                        if (corrupted)
                            Debug.WriteLine("Image Downloading is Imcomplete (Corrupted)...");

                        // recheck if the file existed and if its corrupted
                        file = await IllustrationFolder.TryGetItemAsync(localName) as StorageFile;
                        ...
                        try
                        {
                            ...
                            {
                                var prop = await file.GetBasicPropertiesAsync();
                                if (prop.Size < expectedSize) file = null;
                            }
                            if (file == null)
                        ...
                        if (fileStream == null)
                            Debug.WriteLine("Illustration file <" + localName + "> is in use.");
                        ...
                        if (fileStream != null && !corrupted)
                        {
                            await memeryStream.AsStreamForRead().CopyToAsync(fileStream.AsStreamForWrite());
                            await fileStream.FlushAsync();
                            progress.Report(95);
                            memeryStream.Seek(0);
                        }
                        imageStream = memeryStream;
                    }
                }
                catch (Exception excp)
                {
                    Debug.WriteLine("Unrecoverale exception in caching image, detail : " + excp.Message);
                    throw excp;
                }
            }
            else
        ...
        {
            return GetAsync(CacheFolder, "popular_series.json", DmzjDocSecBase.GetFeaturedBooks, TimeSpan.FromDays(1), forceRefresh);
        }
